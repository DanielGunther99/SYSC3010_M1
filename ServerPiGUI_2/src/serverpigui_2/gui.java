...
class Refresh extends Thread {
    ...
    public void run() {
        try {
            ...
            if (strTime.equals("") && strDate.equals("")) {
                ...
                while (!stopFlag) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            while (r.next()) {
                                String[] dt = r.getString("datetime").split("/");
                                dbout += (dt[0] + "\t"
                                        + dt[1] + "\t"
                                        + r.getInt("bpm") + "\t"
                                        + r.getInt("ppm") + "\t"
            ...
            } else if (!strTime.equals("") && strDate.equals("")) {
                // Bring up the requested minute of data using todays date
                java.sql.Date currDate = valueOf(LocalDate.now().atStartOfDay().toString().split("'")[0]);
                String dbout = "";
                java.sql.Time reqTime_delay = null;
                java.sql.Time reqTime = null;
                try {
                    reqTime = new java.sql.Time(dateFormat.parse(strTime + ":00").getTime() + currDate.getTime());
                    long reqTime_1 = reqTime.getTime() + 60000; // Add one minute
                    reqTime_delay = new java.sql.Time(0, 0, 0);
                    reqTime_delay.setTime(reqTime_1);
                ...
                try {
                    ...
                    while (r.next()) {
                        ...
                        java.sql.Time dataTime = new java.sql.Time(timeFormat.parse(dt[1]).getTime());
                        java.sql.Date dataDate = valueOf(dt[0]);
                        
                        System.out.println("4");
                        if (dataDate.equals(currDate) && dataTime.after(reqTime) && dataTime.before(reqTime_delay)) {
                    ...
                    while (!stopFlag) {
                        String conout = "Current ID: " + id + "\nViewing time: " + strTime + "\nKnownIDs: ";
                        comm = "SELECT * FROM knownIDs";
                        r = command.executeQuery(comm);
                        while (r.next()) {
                            int a = r.getInt("id");
